...
# add the header and logo from the prototype.
...
# Add all components to load user input data. 
# In this prototype, we use selectbox() to create a widget that allows users to select one of several available options. 
# In addition, we also use number_input() to generate a widget that accepts numerical input from users.
...
# Adding an expander layout that allows users to view or hide the dataframe that has been created.
with st.expander("View the Raw Data"):
    st.dataframe(data=data, width=800, height=10)
    
# Add an interactive button named Predict using the button() function provided by Streamlit.
